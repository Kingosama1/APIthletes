...
export const getAllSessions = async (req: Request, res: Response) => {
  try {
    ...
    const sessions = await ClassSession.find(query).populate("trainer", "name email");
...
export const getSessionById = async (req: Request, res: Response) => {
  try {
    const session = await ClassSession.findById(req.params.id).populate("trainer", "name email");
...
export const updateSession = async (req: Request, res: Response) => {
  try {
    ...
    if (session.trainer.toString() !== (req as any).user.id) {
      return res.status(403).json({ message: "Forbidden: You are not the trainer of this session" });
    }
